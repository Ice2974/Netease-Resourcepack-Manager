...
- 导入并校验 `.zip` / `.mcpack`。
- 执行替换，并严格保留目标资源包原 `manifest.json`；替换模式包括：全量替换、覆盖合并、仅新增。
- 替换前自动备份，失败时自动回滚，支持手动回滚最近一次替换前状态。
- 记录操作日志，并提供日志目录查看入口。
- 删除资源包：支持单个删除、全部删除、批量选择后按 `Delete` 删除；删除必须移入 Windows 回收站，不得直接永久删除。
- 深色模式跟随 Windows「默认应用模式」自动切换，浅色模式为默认降级。
...
- 替换前必须创建目标资源包目录的完整备份。
- 替换模式默认为“全量替换”，由 UI 在执行替换时传入 service 层，service 不在 UI 层之外自行猜测模式。
- 三种替换模式的差异：
  - 全量替换（默认）：先删除目标目录中除原 `manifest.json` 外的所有文件和目录，再复制导入包内容（跳过导入包根 `manifest.json`）。
  - 覆盖合并：不清空目标目录，复制导入包内容（跳过导入包根 `manifest.json`），同名文件覆盖，目标中导入包没有的旧文件保留。
  - 仅新增：不清空目标目录，复制导入包内容（跳过导入包根 `manifest.json`），目标已存在的同名文件/目录跳过不覆盖，仅写入不存在的文件。
- 所有模式都必须保留目标包原 `manifest.json`，永远不得用导入包的 `manifest.json` 覆盖目标 `manifest.json`。
- 复制阶段遇到文件/目录类型冲突（如同名路径一边是文件一边是目录，或父路径是文件导致无法创建目录）时，必须视为失败并触发自动回滚，不得静默删除目录或改写结构。
- 任一步失败必须自动回滚，避免半替换状态；三种模式共用同一条备份/回滚路径。
- 手动回滚默认只回滚最近一次替换前状态。
- 回滚过程、失败原因、本次使用的替换模式和关键异常必须写入日志。
...
- `Delete` 键删除当前选中资源包：
  ...
  - 1 个选中时走单个删除确认流程。
  - 多个选中时走批量删除确认流程。
  - 焦点在输入框、文本框等可编辑控件中时，`Delete` 键不应触发资源包删除。
- 深色模式相关的 UI 修改不得破坏浅色模式下的正常显示与交互。

## 样式与主题规则

- QSS 文件（`app/ui/styles.qss`、`app/ui/styles_dark.qss`）必须兼容源码运行与 PyInstaller `onedir` 打包运行两种场景。
- 深色模式通过读取 Windows「默认应用模式」注册表值决定，读取失败或非 Windows 平台一律降级为浅色模式。
- 若使用 Windows DWM API 为原生标题栏 / `QMessageBox` 等弹窗应用深色标题栏，必须在 API 不可用、句柄无法获取或系统版本不支持时静默失败，不得因此阻断程序启动或影响浅色模式下的默认外观。
- 自定义绘制（如卡片 delegate）应按系统应用模式选配配色，避免深色主题下出现白底卡片、浅色外框或系统 focus 虚线框残留。
